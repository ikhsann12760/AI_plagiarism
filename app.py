...
# Fungsi klasifikasi teks AI/Human dengan Sapling.ai API
import requests

def classify_text(text):
    api_key = os.getenv('SAPLING_API_KEY')
    if not api_key:
        logging.error("SAPLING_API_KEY environment variable not set.")
        return 0
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return round(score * 100, 2)  # 0=human, 100=AI
        else:
            logging.error(f"Sapling API error: {response.status_code} {response.text}")
            return 0
    except Exception as e:
        logging.error(f"Sapling API request failed: {e}")
        return 0
